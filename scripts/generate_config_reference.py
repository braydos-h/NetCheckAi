...
SUBSYSTEM_BY_TOP = {
    "adaptive_exploits": "exploit", "agent": "agent", "api": "api/webui",
    "autonomous": "autonomous", "benchmark": "benchmark", "browser": "browser",
    "caldera": "ad", "chatgpt": "providers", "cve_lookup": "research/cve",
    "embeddings": "embeddings", "engine_mcp": "mcp/engine", "eval": "eval",
    "exploit": "exploit/agent", "fsm": "fsm", "hitl": "hitl", "ics": "ics",
    "killchain": "killchain", "long_session": "long_session", "mcp": "mcp",
    "memory": "memory", "mitre": "mitre", "models": "models",
    "multi_model": "multi-model", "nmap": "recon", "ollama": "models/provider",
    "opencode_go": "providers", "operator_connection": "operator",
    "opsec": "opsec", "orchestrator": "autonomous",
    "outcome_judgment": "eval", "plugins": "plugins",
    "poc_verification": "poc", "providers": "providers",
    "reasoning": "reasoning", "recon": "recon", "replay_simulator": "—",
    "research": "research", "sandbox": "sandbox", "skills": "skills",
    "snapshots": "snapshots", "stealth": "opsec", "swarm": "swarm",
    "threat_intel": "threat_intel", "ticketing": "ticketing",
    "webhook_notify": "plugins", "witness": "witness",
}
...
RESTART_YES_TOPS = {"api", "models", "mcp", "engine_mcp", "ollama",
                    "chatgpt", "opencode_go", "providers", "embeddings"}

PRESERVED = ("allowed", "consumers", "effect", "env", "restart",
             "subsystem", "tests")
...
def load_schema_defaults() -> dict:
    ...
    for node in tree.body:
        if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name) \
                and node.target.id == "CONFIG_SCHEMA":
            return ast.literal_eval(node.value)
        if isinstance(node, ast.Assign) and any(
                isinstance(t, ast.Name) and t.id == "CONFIG_SCHEMA"
...
def split_row(line):
    ...
    src_idx = next((i for i, c in enumerate(cells)
                    if c.startswith("`config.yaml")), None)
    if src_idx is None:
        return cells
...
def parse_existing(text):
    ...
    for line in text.splitlines():
        ...
        rows[key] = {
            "type": cells[1], "default": cells[2], "allowed": cells[3],
            "source": cells[4], "consumers": cells[5], "effect": cells[6],
            "env": cells[7], "restart": cells[8], "subsystem": cells[9],
...
def fmt_type(lab, sch):
    ...
    if isinstance(sample, list):
        if sample and all(isinstance(x, str) for x in sample):
            return "`list[str]`"
        if sample and all(isinstance(x, int) and not isinstance(x, bool)
                          for x in sample):
            return "`list[int]`"
        return "`list`"
...
def main():
    ...
    for key in sorted(lab_leaves):
        ...
        if old:
            row = {"type": fmt_type(lab, schema_leaves.get(key)),
            ...
            row = {"type": fmt_type(lab, schema_leaves.get(key)),
                   "default": fmt_default(key, lab, schema_leaves),
                   "allowed": "—", "source": source, "consumers": "",
                   "effect": "—", "env": "—",
                   "restart": "yes" if top in RESTART_YES_TOPS else "no",
                   "subsystem": subsystem_for(key), "tests": "—"}
        out_rows.append(
    ...
    hdr = next(i for i, ln in enumerate(text_lines)
               if ln.startswith("| Key |"))
    end = next(i for i, ln in enumerate(text_lines[hdr:], hdr)
               if not text_lines[i].startswith("|"))
    head = text_lines[:hdr]
    tail = text_lines[end:]
    head = [re.sub(r"\(\d{4}-\d{2}-\d{2}\)", f"({today})", ln) for ln in head]
    head = [re.sub(r"_Generated \d{4}-\d{2}-\d{2} from `config\.yaml` "
                   r"\(\d+ leaf keys, \d+ top-level blocks\)",
                   f"_Generated {today} from `config.yaml` "
                   f"({len(lab_leaves)} leaf keys, {n_blocks} top-level blocks)",
                   ln) for ln in head]
    ...
    print(f"keys={len(lab_leaves)} blocks={n_blocks} "
          f"new={n_new} lab_only={n_lab_only} date={today}")
